    ...
    BaseViewModel<TraineeConnectUiState, TraineeConnectUiEvent, TraineeConnectSideEffect>(
        ...
        override suspend fun handleEvent(event: TraineeConnectUiEvent) {
            when (event) {
                ...
                is TraineeConnectUiEvent.OnValidateClick -> validateCode(event.code)
        ...
        private fun validateCode(code: String) {
            // TODO 코드 유효성 확인
            if (code.length == 8) {
                updateState { copy(inviteCode = code, isCodeValid = true) }
            } else {
                updateState { copy(inviteCode = code, isCodeValid = false) }
            }
        }
